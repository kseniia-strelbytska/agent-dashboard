...
def main():
            ...
            for show_all in (False, True):
                for help_open in (False, True):
                    ...
                    for i, line in enumerate(dash.compose(data)):
                        ...
                        if w > cols:
                            failures.append(
                                "%s cols=%d open=%s all=%s help=%s line %d: width %d > %d\n    %r"
                                % (label, cols, opened, show_all, help_open, i, w, cols,
                                   ANSI.sub("", line.text)))
    data = variants["healthy"]

    # opening a row must not move the row itself
    for cols in (80, 120):
        dash = tui.Dashboard()
        dash.cols, dash.rows = cols, 400
        ...
        dash.open_id = "s1"
        opened_lines = dash.compose(data)
        moved = next(i for i, l in enumerate(opened_lines) if l.session_id == "s1")
        if moved != target:
            failures.append("cols=%d: opening s1 moved its own first line %d -> %d"
                            % (cols, target, moved))
    ...
    if "s0" not in owners:
        failures.append("opened session has no owned lines")
    ...
    if failures:
        ...
        for f in failures:
            print(" -", f)
        return 1
    print("layout ok: widths, expansion stability and hit-testing all hold")
    return 0
